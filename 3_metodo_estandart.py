...
def entry(): #Es la entrada para que la interfaz de tkinter nos ayude a ingresar las variables
    ...
    z_list = []
    H_list=[]
    with open ('3_metodo_estandart.txt', 'w') as file:
        file.write(" X            y            Area             P            n          V            Rh             Sf             E             Fr       sfm             Dh       Z           H\n")
        saltos_X=-500 #float(input("Escriba el salto constante en x (metros): "))
        for x in range(0,-8001,saltos_X):
            if x ==0: #primer ciclo
                z=0
                sfm=0
                if H3>=H2:
                ...
                while True:
                    ...
                    if H3 < H2:
                        if y <= H3:
                            ...
                            H=z+y+v2
                            Hf=H
                    if abs(H_list[i]-H)<=0.0001:
                        saux=s# presentaba error
                        sfm=(s+saux)/2
                        break
                    y+=0.0001
            ...
            y_list.append(y)
            H_list.append(H)
            file.write('%12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f %12.7f\n' % (x,y,areat,pt,nequiv,v,rh,s,Es,fr,sfm,dh,z,H))
        
    data = {'X':x_list, 'Y':y_list,'Area':area_list,'Perimetro':perimetro_list, 'n equiv':n_list, 'Rh':rh_list, 'Dh':dh_list}
    data2 = {'T':t_list,'Sf':s_list, 'V':v_list,'E':es_list,'Fr':fr_list, 'Sfm':sfm_list, 'Z':z_list, 'H':H_list}
    data_df = pd.DataFrame(data)
    data2_df = pd.DataFrame(data2)
    print()
    print('Graficas y Tabla de valores')
    print()
    print(data_df) 
    print()
    print(data2_df)   
    ...
    plt.title('Perfil paso estandar')
    plt.ylabel('')
    ...
    #------------------------------CREAR VENTANA---------------------------------
    root = tkinter.Tk()
    root.wm_title("GRAFICA METODO PASO ESTANDAR")
    #------------------------------CREAR GRAFICA---------------------------------
...
interfaz.title("CALCULO METODO PASO ESTANDAR")
...
mimage=PhotoImage(file="3_metodo_estandart.png") #Ingresar la imagen llamada:
...
hh222=Label(miframe,text="METODO PASO ESTANDAR")
